import {test, stub} from 'supertape';
import {release} from './release.js';

test('wisdom: release: emit', async (t) => {
    const version = 'v1.0.0';
    const count = 1;
    const info = {
        release: false,
    };
    
    const emit = stub();
    
    const emitter = {
        emit,
    };
    
    await release({
        version,
        info,
        chlog: stub().returns('xxx'),
        emitter,
        count,
    });
    
    const expected = [
        'data',
        'release: false\n',
    ];
    
    t.calledWith(emit, expected);
    t.end();
});

test('wisdom: release: emit: no repository', async (t) => {
    const version = 'v1.0.0';
    const count = 1;
    const info = {};
    
    const emit = stub();
    const emitter = {
        emit,
    };
    
    await release({
        version,
        info,
        chlog: stub().returns('xxx'),
        emitter,
        count,
    });
    
    const expected = `Error releasing on github: no 'repository' field found in 'package.json'`;
    
    const [arg] = emit.args;
    const [, second] = arg;
    
    t.match(second, expected);
    t.end();
});

test('wisdom: release: repository.url: no chlog', async (t) => {
    const version = 'v1.0.0';
    const count = 1;
    const info = {
        name: 'wisdom',
        release: true,
        repository: {
            url: 'https://github.com/coderaiser/wisdom',
        },
    };
    
    const grizzly = stub().rejects(Error('Validation Failed: {"resource":"Release","code":"already_exists","field":"tag_name"}'));
    
    const emit = stub();
    const emitter = {
        emit,
    };
    
    await release({
        version,
        info,
        chlog: stub(),
        emitter,
        count,
        grizzly,
    });
    
    const expected = `Error releasing on github: Validation Failed: {"resource":"Release","code":"already_exists","field":"tag_name"}`;
    
    const [, , arg] = emit.args;
    const [, second] = arg;
    
    t.match(second, expected);
    t.end();
});

test('wisdom: release: repository.url', async (t) => {
    const version = 'v1.0.0';
    const count = 1;
    const info = {
        name: 'wisdom',
        release: true,
        repository: {
            url: 'https://github.com/coderaiser/wisdom',
        },
    };
    
    const grizzly = stub().rejects(Error('Validation Failed: {"resource":"Release","code":"already_exists","field":"tag_name"}'));
    
    const emit = stub();
    const emitter = {
        emit,
    };
    
    await release({
        version,
        info,
        chlog: stub().returns(''),
        emitter,
        count,
        grizzly,
    });
    
    const expected = `Error releasing on github: Validation Failed: {"resource":"Release","code":"already_exists","field":"tag_name"}`;
    
    const [, , arg] = emit.args;
    const [, second] = arg;
    
    t.match(second, expected);
    t.end();
});

test('wisdom: release: repository.url: no error', async (t) => {
    const version = 'v1.0.0';
    const count = 1;
    const info = {
        name: 'wisdom',
        release: true,
        repository: {
            url: 'https://github.com/coderaiser/wisdom',
        },
    };
    
    const grizzly = stub().resolves();
    
    const emit = stub();
    const emitter = {
        emit,
    };
    
    await release({
        version,
        info,
        chlog: stub().returns(''),
        emitter,
        count,
        grizzly,
    });
    
    const expected = [
        'data',
        `release: ok\n`,
    ];
    
    t.calledWith(emit, expected);
    t.end();
});

test('wisdom: release: repository.url: exist', async (t) => {
    const version = 'v1.0.0';
    const count = 1;
    const info = {
        name: 'wisdom',
        release: true,
        repository: {
            url: 'https://github.com/coderaiser/wisdom',
        },
    };
    
    const grizzly = stub().resolves();
    
    const emit = stub();
    const emitter = {
        emit,
    };
    
    await release({
        version,
        info,
        chlog: stub().returns(''),
        emitter,
        count,
        grizzly,
    });
    
    const expected = [
        'data',
        `release: ok\n`,
    ];
    
    t.calledWith(emit, expected);
    t.end();
});
