...
function publish(version, info, emitter) {
    ...
    series([
        function beforePublish(callback) {
            ...
            runWisdom('wisdom', '', version, info.scripts, emitter, callback);
        },
        ...
        function beforePublishType(callback) {
            ...
            runWisdom('wisdom:type', type, version, info.scripts, emitter, callback);
        },
        ...
        function afterPublish(callback) {
            ...
            runWisdom('wisdom:done', '', version, info.scripts, emitter, callback);
        },
    ], (error) => {
        if (error)
            emitter.emit('error', error);
...
function runWisdom(name, type, version, scripts, emitter, callback) {
    const cmd = redrun(name, scripts) + ' ' + type;
